...
const minDraft = ref<number | undefined>(props.priceMin)
const maxDraft = ref<number | undefined>(props.priceMax)

watch(() => props.search, value => { searchDraft.value = value })
watch(() => props.priceMin, value => { minDraft.value = value })
watch(() => props.priceMax, value => { maxDraft.value = value })
  ...
  <div class="flex flex-col gap-7">
    ...
    <details v-if="facets?.price_range" open class="group border-t border-neutral-200 pt-6">
      ...
      <div class="mt-4 grid grid-cols-2 gap-2">
        <UInputNumber v-model="minDraft" :min="0" :placeholder="String(Math.floor(facets.price_range.min))" size="md" />
        <UInputNumber v-model="maxDraft" :min="0" :placeholder="String(Math.ceil(facets.price_range.max))" size="md" />
      </div>
      <UButton label="Apply price" color="neutral" variant="outline" block class="mt-3" @click="emit('price', minDraft, maxDraft)" />
    </details>
