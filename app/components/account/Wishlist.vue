    ...
    <div v-else-if="items.length" class="mt-8 grid grid-cols-2 gap-x-4 gap-y-8 md:grid-cols-3 lg:grid-cols-4">
      <article v-for="item in items" :key="item.id" class="group min-w-0">
        <div class="relative aspect-[4/5] overflow-hidden bg-[#eee9e1]">
          <NuxtLink :to="`/product/${item.slug}`" class="block h-full"><img :src="item.thumb || PRODUCT_IMAGE_FALLBACK" :alt="item.name" class="h-full w-full transition duration-500 group-hover:scale-[1.025]" :class="item.thumb ? 'object-cover' : 'object-contain'"></NuxtLink>
