...
import { applyCatalogImageFallback, catalogImageUrl } from '~/utils/catalog'
            ...
            <article v-for="product in products" :key="product.id" class="group min-w-0">
              <div class="relative aspect-[4/5] overflow-hidden bg-neutral-100">
                <NuxtLink :to="`/product/${product.slug}`" class="block h-full" :aria-label="`View ${product.name}`">
                  <img :src="catalogImageUrl(product.images)" :alt="product.name" class="h-full w-full transition duration-500 group-hover:scale-[1.025]" :class="hasCatalogImage(product.images) ? 'object-cover' : 'object-contain'" loading="lazy" width="720" height="900" @error="applyCatalogImageFallback">
                </NuxtLink>
